...
fn decimal() -> Result<()> {
    ...
    let expected = Int128Array::from(&[Some(11), Some(12), None, Some(13)]).to(DataType::Decimal(
        DecimalType::Int128,
        2,
        1,
    ));
...
fn decimal_only_scale() -> Result<()> {
    let result = test_deserialize(
        "0.01,\n0.12,\n0.222,\n0.13,\n",
        DataType::Decimal(DecimalType::Int128, 2, 2),
    )?;
    let expected = Int128Array::from(&[Some(1), Some(12), None, Some(13)]).to(DataType::Decimal(
        DecimalType::Int128,
        2,
        2,
    ));
...
fn decimal_only_integer() -> Result<()> {
    ...
    let expected = Int128Array::from(&[Some(1), Some(1), None, Some(10)]).to(DataType::Decimal(
        DecimalType::Int128,
        1,
        0,
    ));
